<?php

// Verificar si hay sesión activa y permisos
if (!isset($_SESSION['user_id']) || (!hasPermission(ROL_ADMIN) && !hasPermission(ROL_CONTABILIDAD))) {
    setFlashMessage('No tiene permisos para acceder a esta sección', 'danger');
    header('Location: ' . BASE_URL);
    exit;
}

// Verificar que se haya enviado el ID de la planilla
if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !isset($_POST['id_planilla'])) {
    setFlashMessage('Solicitud inválida', 'danger');
    header('Location: ' . BASE_URL . '?page=planillas/lista');
    exit;
}

$id_planilla = intval($_POST['id_planilla']);

if ($id_planilla <= 0) {
    setFlashMessage('ID de planilla inválido', 'danger');
    header('Location: ' . BASE_URL . '?page=planillas/lista');
    exit;
}

// Verificar que la planilla exista y esté en estado borrador
$db = getDB();
try {
    $query = "SELECT * FROM planillas WHERE id_planilla = :id_planilla";
    $stmt = $db->prepare($query);
    $stmt->bindParam(':id_planilla', $id_planilla, PDO::PARAM_INT);
    $stmt->execute();
    
    if ($stmt->rowCount() == 0) {
        setFlashMessage('La planilla especificada no existe', 'danger');
        header('Location: ' . BASE_URL . '?page=planillas/lista');
        exit;
    }
    
    $planilla = $stmt->fetch(PDO::FETCH_ASSOC);
    
    if ($planilla['estado'] != 'Borrador') {
        setFlashMessage('Solo se pueden aprobar planillas en estado borrador', 'warning');
        header('Location: ' . BASE_URL . '?page=planillas/ver&id=' . $id_planilla);
        exit;
    }
    
    // Aprobar la planilla (cambiar el estado a Aprobada)
    $queryUpdate = "UPDATE planillas SET 
                   estado = 'Aprobada', 
                   fecha_aprobacion = NOW(),
                   aprobado_por = :aprobado_por
                   WHERE id_planilla = :id_planilla";
    
    $stmtUpdate = $db->prepare($queryUpdate);
    $stmtUpdate->bindParam(':aprobado_por', $_SESSION['user_id'], PDO::PARAM_INT);
    $stmtUpdate->bindParam(':id_planilla', $id_planilla, PDO::PARAM_INT);
    
    if ($stmtUpdate->execute()) {
        // Registrar la acción en el historial
        registrarHistorial('Aprobación de planilla', 'Se aprobó la planilla #' . $id_planilla, 'planillas', $id_planilla);
        
        setFlashMessage('Planilla aprobada correctamente', 'success');
    } else {
        setFlashMessage('Error al aprobar la planilla', 'danger');
    }
    
} catch (Exception $e) {
    setFlashMessage('Error: ' . $e->getMessage(), 'danger');
}

// Redireccionar a la vista de la planilla
header('Location: ' . BASE_URL . '?page=planillas/ver&id=' . $id_planilla);
exit;

/**
 * Función para registrar acciones en el historial del sistema
 * @param string $accion Título de la acción
 * @param string $descripcion Descripción detallada
 * @param string $tipo_entidad Tipo de entidad (planillas, empleados, etc.)
 * @param int $id_entidad ID de la entidad
 * @return bool Éxito de la operación
 */
function registrarHistorial($accion, $descripcion, $tipo_entidad, $id_entidad) {
    try {
        $db = getDB();
        $query = "INSERT INTO historial (accion, descripcion, tipo_entidad, id_entidad, usuario_id, fecha)
                 VALUES (:accion, :descripcion, :tipo_entidad, :id_entidad, :usuario_id, NOW())";
        
        $stmt = $db->prepare($query);
        $stmt->bindParam(':accion', $accion);
        $stmt->bindParam(':descripcion', $descripcion);
        $stmt->bindParam(':tipo_entidad', $tipo_entidad);
        $stmt->bindParam(':id_entidad', $id_entidad);
        $stmt->bindParam(':usuario_id', $_SESSION['user_id']);
        
        return $stmt->execute();
    } catch (Exception $e) {
        // Si hay error, solo lo ignoramos para no interrumpir el flujo principal
        return false;
    }
} 